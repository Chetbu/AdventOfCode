...
def correct_update(ordering_rule_list, update):
    current_page_index = 0
    w_update = copy.copy(update)
    res = True
    while res and current_page_index < len(w_update) - 1:
        compared_page_index = current_page_index + 1
        res_page = True
        while compared_page_index < len(w_update) and res_page:
            res_page = check_is_in_order(ordering_rule_list, w_update[current_page_index], w_update[compared_page_index])
            compared_page_index += 1
        
        if res_page:
            current_page_index += 1
        else:
            #invert the two pages
            compared_page_index -= 1
            current_page_value = w_update[current_page_index]
            w_update[current_page_index] = w_update[compared_page_index]
            w_update[compared_page_index] = current_page_value
            #print(current_page_index, compared_page_index, w_update[current_page_index], w_update[compared_page_index])
    return w_update
...
while update_index < len(page_number_list):
    ...
    if not(check_update(ordering_rule_list, current_update)):
        w_update = correct_update(ordering_rule_list, current_update)
        #print(w_update)
        result += w_update[int((len(w_update) - 1)/2)]
    update_index += 1
