def move_crate(arr, start:int, end:int):
    new_arr = arr
    crate = arr[start][len(arr[start])-1:]
    new_arr[end].append(crate[0])
    new_arr[start] = arr[start][:len(arr[start])-1]
    return new_arr
...
with open('data_2.txt') as f:
    ...
    with open('data.txt') as g:
        ...
        for move in moves:
            ...
            i=0
            while i < iteration:
                pile_array = move_crate(pile_array,start -1,end -1)
                i = i + 1
