...
def is_better(c_s: State, c_ref: State):
    str_arr = ["ore", "clay", "obs", "geode"]
    if c_s.round > c_ref.round:
        return False
    elif not(more_ressources(c_s.res, c_ref.res)):
        return False
    else:
        for s in str_arr:
            if getattr(c_ref, s +"_robot") > getattr(c_s, s +"_robot"):
                return False
    return True 
...
while b_id < len(blueprint_arr):
    ...
    #print(c_state)
    while len(state_array) > 0 and count < 1000000:
        count +=1
    ...
    b_id += 1
    quality_sum = quality_sum + b_id * max_geode
