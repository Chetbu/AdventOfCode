...
with open('data.txt') as f:
    ...
    while len(move_backlog)>0:
        ...
        if c_cell[0] > 0 and (cell_height - lines_array[c_cell[0]-1][c_cell[1]])<=1 and step_array[c_cell[0]-1][c_cell[1]] < 0: ## if it can go up and not visited
  
            step_array[c_cell[0]-1][c_cell[1]] = cell_value + 1 #set the value
            move_backlog.append([c_cell[0]-1,c_cell[1]])

        if c_cell[0] < len(step_array)-1 and (cell_height - lines_array[c_cell[0]+1][c_cell[1]])<=1 and step_array[c_cell[0]+1][c_cell[1]] < 0: ## if it can go down and not visited
  
            step_array[c_cell[0]+1][c_cell[1]] = cell_value + 1 #set the value
            move_backlog.append([c_cell[0]+1,c_cell[1]])

        if c_cell[1] > 0 and (cell_height - lines_array[c_cell[0]][c_cell[1]-1])<=1 and step_array[c_cell[0]][c_cell[1]-1] < 0: ## if it can go left and not visited
  
            step_array[c_cell[0]][c_cell[1]-1] = cell_value + 1 #set the value
            move_backlog.append([c_cell[0],c_cell[1]-1])

        if c_cell[1] < len(step_array[0])-1 and (cell_height - lines_array[c_cell[0]][c_cell[1]+1])<=1 and step_array[c_cell[0]][c_cell[1]+1] < 0: ## if it can go right and not visited
    ...
    while x < len(step_array):
        ...
        while y < len(step_array[0]):
            if lines_array[x][y] == ord("a") and step_array[x][y] > 0:
