...
c = 1
circuit_l_array = []
while c < max_index_circuit:
    count = 0
    for k in circuit_dict.keys():
        if circuit_dict[k] == c:
            count += 1
    print("circuit ", c, " has ", count)
    circuit_l_array.append(count)
    c += 1

circuit_l_array.sort(reverse=True)
print(circuit_l_array[0] * circuit_l_array[1]* circuit_l_array[2])
